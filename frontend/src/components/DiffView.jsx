...
export default function DiffView({ jd, cvText, matchedSkills, missingSkills }) {
  ...
  const renderParts = (parts) => parts.map((part, i) => {
    if (part.type === 'green') return <mark key={i} className="bg-emerald-900 text-emerald-300 rounded px-0.5">{part.text}</mark>
    if (part.type === 'red') return <mark key={i} className="bg-red-900 text-red-300 rounded px-0.5">{part.text}</mark>
  ...
  return (
    <div className="sm:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
        ...
        <p className="text-xs text-zinc-300 whitespace-pre-wrap leading-relaxed">{renderParts(jdParts)}</p>
      </div>
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-5">
        <p className="text-xs font-semibold text-zinc-500 uppercase tracking-widest mb-3">Your CV</p>
        <p className="text-xs text-zinc-300 whitespace-pre-wrap leading-relaxed">{renderParts(cvParts)}</p>
      </div>
    </div>
  )
}